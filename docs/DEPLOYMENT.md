...
- Public endpoints covered by PHP module:
  ...
  - `/api/leads/careers`
- Admin APIs are intentionally out of scope for this phase.
...
- Runtime routes are explicitly marked `prerender = false` for:
  - `/admin/*`
  - `/api/*`
  - Runtime-dependent content pages using admin-managed data.
...
3. Confirm admin login/logout/content update works in deployed environment.
