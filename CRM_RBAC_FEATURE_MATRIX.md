...
The Income KPI includes settled invoice ledger entries and settled `income`/`revenue` rows. Pending, draft, sent, overdue, and half-payment invoice entries remain outstanding and do not increase Income or Available balance; cancelled invoices are excluded. The Financial Management summary also exposes outstanding expense obligations for unpaid `expense` rows; those rows do not reduce Available balance until settled. Finance is company-staff only; clients have no finance-table policy.
