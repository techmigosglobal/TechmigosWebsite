...
const HeroSection = memo(function HeroSection() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (badgeRef?.current) badgeRef?.current?.classList?.add('active');
    }, 200);
  ...
  return (
    <section className="relative border-b border-border min-h-[100svh] overflow-hidden bg-background pt-16">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100svh-64px)] px-5 py-12 sm:px-10 md:px-16 lg:px-24 text-center">
        {/* Badge */}
        <div ref={badgeRef} className="reveal reveal-delay-1 mb-5 sm:mb-8">
        ...
        {/* Pain-point hook */}
        <p className="reveal reveal-delay-2 mb-4 sm:mb-5 text-sm sm:text-base font-mono text-muted-foreground/80 tracking-wide border border-border/50 rounded-lg px-4 py-2 bg-card/40 backdrop-blur-sm max-w-xl">
        ...
        <h1
          ref={titleRef}
          className="text-4xl xs:text-5xl sm:text-6xl md:text-7xl xl:text-8xl font-bold tracking-tighter leading-none mb-4 sm:mb-7 text-foreground"
        >
        ...
        {/* Solution statement */}
        <p className="reveal reveal-delay-4 max-w-2xl text-sm sm:text-base md:text-lg text-muted-foreground leading-relaxed mb-6 sm:mb-10">
        ...
        <div className="reveal reveal-delay-5 flex flex-col xs:flex-row gap-3 mb-8 sm:mb-14">
          <a
            href="#demo"
            onClick={(e) => {
              e.preventDefault();
        ...
        <div
          className="reveal flex items-center gap-2 text-xs text-muted-foreground font-mono"
